...
export const closeJob = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const job = await Job.findOneAndUpdate(
      { _id: id, employer_id: req.user!.userId },
      { status: "closed" },
      { new: true },
    );
    if (!job) {
...
export const getSavedJobs = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user!.role !== "seeker") {
      return res
        .status(403)
        .json({ message: "Only job seekers can get saved jobs" });
    }
    const userId = req.user!.userId;
    const jobs = await SavedJobs.find({ user_id: userId })
      .populate(
        "job_id",
        "title description salary_max location remote skills status ",
      )
      .lean();
...
export const toggleSaveJob = async (req: AuthRequest, res: Response) => {
  try {
    if (req.user!.role !== "seeker") {
      return res
        .status(403)
        .json({ message: "Only job seekers can save jobs" });
    }
    ...
    if (existing) {
      ...
      return res
        .status(200)
        .json({ message: "Job unsaved successfully", saved: false });
    } else {
      ...
      return res
        .status(201)
        .json({ message: "Job saved successfully", saved: true });
    }
  } catch (error) {
    res.status(500).json({ error: "Server error" });
  }
