...
export const uploadResume = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    tempFilePath = path.join(
      os.tmpdir(),
      `${Date.now()}-${multerReq.file.originalname}`,
    );
...
export const getResume = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const parsedData = await ResumeParsedData.findOne({
...
export const createResume = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!file_url || !file_name) {
      return res
        .status(400)
        .json({ error: "file_url and file_name are required" });
    }
